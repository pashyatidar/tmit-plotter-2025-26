...
export default function RocketStatus({ state, className = '' }: Props) {
    ...
    const getStatusStyles = () => {
        switch (state) {
            ...
            case 2: return { label: "COAST",      color: "text-blue-400",    glow: "shadow-blue-500/40", icon: <Zap size={14}/> };
            case 3: return { label: "DROGUE",     color: "text-purple-400",  glow: "shadow-purple-500/40", icon: <ArrowDown size={14}/> };
            case 4: return { label: "MAIN",       color: "text-emerald-400", glow: "shadow-emerald-500/40", icon: <ArrowDown size={14}/> };
            case 5: return { label: "RECOVERY",   color: "text-red-500",     glow: "shadow-red-500/20", icon: <Lock size={14}/> };
    ...
    return (
        <div className={`h-full bg-slate-900/50 rounded-2xl border border-slate-800 p-2 flex items-center justify-between relative overflow-hidden group transition-all duration-500 ${state === 1 ? 'border-orange-500/30' : ''} ${className}`}>
            ...
            <div className="h-full w-12 flex flex-col items-center justify-center relative z-10 mr-2">
                <div className={`w-3 h-8 my-[1px] rounded-sm transition-all duration-300
                    ${state === 1 ? 'bg-orange-500 drop-shadow-[0_0_15px_rgba(249,115,22,1)] animate-pulse' : 
                      state === 4 ? 'bg-emerald-500 drop-shadow-[0_0_10px_rgba(16,185,129,0.8)]' : 'bg-slate-700'}`}
                />
            </div>
        </div>
    );
