...
// =================================================================================
// MODIFIED: This function now uses conditional logic for the X-axis scale
// =================================================================================
function updateAllPlots() {
    // Update the data for all plots first
    if (mainPlot1.instance) {
    ...
    // Check which mode is active to determine the X-axis scale behavior
    if (randomPlotting || isSerialConnected) {
        // For Random and Serial modes, use a 20-second sliding window
        const windowEndTime = uplotData.time[dataLength - 1];
        const windowStartTime = windowEndTime - 20;
        newScale = { min: windowStartTime, max: windowEndTime };
    } else if (isPlotting) {
        // For CSV mode, show the full range with some padding
        const windowStartTime = uplotData.time[0];
        const windowEndTime = uplotData.time[dataLength - 1];
        const duration = windowEndTime - windowStartTime;
        const padding = duration > 0 ? duration * 0.1 : 1;
        const newMax = windowEndTime + padding;
        newScale = { min: windowStartTime, max: newMax };
    }

    // Apply the determined scale to all plots
    if (newScale) {
